...
class imgbrowse {
  ...
  function __construct() {
    ...
    $this->root = ($_SERVER['HTTP_HOST']=='localhost') ? $ArrUrl[1]."/".trim($this->root, '/') .'/' : trim($this->root, '/') .'/';
  ...
  public function getMenuImgs() {
    ...
    foreach($obdr as $fileobj) {
      ...
      if($fileobj->isFile() && in_array($fileobj->getExtension(), $this->imgext)) $re['imgs'] .= '<span><img src="'. $site . $this->root . $this->imgdr . $name .'" alt="'. $name .'" height="70" title="Pilih" />'. $name .'</span>';
